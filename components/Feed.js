...
const Feed = ({ connected, name, url }) => {
	...
	useEffect(() => {
		toast('Posts Refreshed!', {
			icon: '🔁',
			style: { borderRadius: '10px', backrground: '#252526', color: '#fffcf9' },
		});
	}, [posts.length]);

	const getAllPosts = async () => {
		try {
			const postsData = await program.account.postsAccount.all();
			postsData.sort(
	...
	const savePost = async text => {
		...
		try {
			...
			await program.rpc.createState({
				accounts: { state, authority: wallet.publicKey, ...defaultAccounts },
			});
			return;
		}
